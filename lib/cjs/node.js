...
exports.tokenBoolean = tokenBoolean;
...
function tokenBoolean() {
  return _crypto.default.randomBytes(1)[0] % 2 === 0;
}
...
function seedUuid(seed) {
  ...
  return [hash.subarray(0, 4).toString('hex'), hash.subarray(4, 6).toString('hex'), hash.subarray(6, 8).toString('hex'), hash.subarray(8, 10).toString('hex'), hash.subarray(10, 16).toString('hex')].join('-');
}
